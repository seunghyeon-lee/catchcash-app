...
| 디자인 반영 버전 | 기존 맵 상세 디자인 시안 기준 / Naver Maps Grug Dark 스타일 / 마커·컨트롤 에셋 적용 |
| 주요 기능 | 관리자 등록 보물상자 지도 표시, 내 위치 표시, 100m 기준 거리 안내, 힌트 팝업 연결 |
...
사용자는 자신의 현재 위치와 보물상자 위치를 확인하고, 100m 이내까지는 거리 안내를 받는다. 100m 이내부터는 정확한 거리 숫자 대신 힌트를 활용해 직접 찾으며, 사냥하기 버튼은 거리와 관계없이 AR 사냥 화면으로 진입한다.
...
- 100m 이내 진입 여부를 기준으로 정확한 거리 안내 종료 상태를 판단한다.
- 실제 상자 클릭/오픈 가능 여부는 AR 화면에서 현재 위치와 보물 좌표를 다시 계산해 판단한다.
...
- 힌트를 확인한 뒤 사냥하기를 눌러 AR 사냥 화면으로 이동한다.
...
기존 디자인 시안 기준 맵 상세 화면은 아래 구조를 따른다.
...
| 다음 화면 | 화면 ID | 설명 |
|---|---|---|
| 힌트 팝업/바텀시트 | `06_Treasure_Hint_Bottom_Sheet` | 보물 상세 힌트와 100m 기준 거리 안내 확인 |
...
캐치캐쉬는 AR 화면에서 최종 거리 판정을 다시 수행하므로 가능한 고정밀 위치를 사용한다.
...
지도 화면의 100m 기준은 사냥 가능 거리나 보물 오픈 가능 거리가 아니다.
사용자가 보물 근처에 도착했음을 알려주고, 이후에는 정확한 거리 숫자 대신 힌트를 이용해 직접 찾게 하는 안내 종료 기준이다.

### 16.2 지도 화면에서의 판단

지도 상세 화면은 100m 초과일 때만 실제 거리 숫자를 안내한다.
100m 이내에서는 정확한 거리 숫자를 숨기고 `보물 근처에 도착했어요.` 계열 문구를 표시한다.
사냥하기 버튼은 거리와 관계없이 AR 사냥 화면으로 이동한다.
...
AR 화면 거리 계산
→ treasure.radius_m 기준 실제 상자 클릭/오픈 가능 여부 판단
```
...
|---:|---|
| 100m 초과 | 실제 거리 숫자 표시 |
| 100m 이내 | 정확한 거리 숫자 숨김, 힌트 탐색 안내 |
| 위치 없음 | 위치 확인 필요 안내, 기본 지도 유지 |
...
→ AR 화면에서 현재 위치와 treasure.radius_m 기준으로 실제 상자 클릭/오픈 가능 여부 재판정
```
...
- AR 진입 전 힌트 바텀시트는 100m 기준 거리 안내만 제공하고, 실제 거리 제한은 AR 화면에서 다시 확인한다.
