...
AR 사냥 화면은 사용자가 지도 또는 보물 힌트 단계에서 `사냥하기`를 눌러 진입하는 보물 획득 화면이다.
지도/힌트 단계에서는 AR 진입을 거리로 막지 않으며, 실제 상자 클릭/오픈 가능 여부는 AR 화면에서 현재 위치와 보물 좌표 간 거리를 다시 계산해 판단한다.
